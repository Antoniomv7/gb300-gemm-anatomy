...
* P4.3 analysis-code commit `2ef1ac52907c407dd43c41661382fc8d5673cce4` (the commit whose code produced this bundle), worktree clean: `true`.
...
**How to read these candidates.** The tested grid is exactly 16, 32, 64 KiB of bytes in flight, so `64` is the **upper boundary of the evaluated grid**, not a measured architectural limit: a candidate saturation point is an upstream selection made strictly within the tested grid, never a measured architectural limit and never a universal saturation threshold. Here every group's candidate equals 64, the largest tested bytes-in-flight value in KiB, and the timing-derived effective transfer rate is still rising across the final tested step in every group, so no plateau was observed within the tested range. When the selected candidate equals the largest tested grid point, the selection has reached the boundary of the evaluated range: the tested grid cannot distinguish a genuine saturation point there from one that lies beyond the largest value tested, and every campaign evaluates the same frozen grid, so agreement between the three campaigns on a candidate that sits at that grid's upper boundary is not by itself evidence that a saturation point was independently located.
...
|---|---|---|---|---|---|---|---|---|
| 0 | ldgsts | 2 | 16 | 1.000040180 | 1.000008917 | 1.000095929 | HBM_VALIDATED | none recorded / none recorded / none recorded |
| 1 | tma | 2 | 16 | 1.000055527 | 1.000036559 | 1.000092229 | HBM_VALIDATED | none recorded / none recorded / none recorded |
| 2 | tma | 4 | 32 | 1.000020400 | 1.000004057 | 1.000047913 | HBM_VALIDATED | none recorded / none recorded / none recorded |
| 3 | ldgsts | 4 | 32 | 1.000007936 | 1.000003002 | 1.000011532 | HBM_VALIDATED | none recorded / none recorded / none recorded |
| 4 | ldgsts | 8 | 64 | 1.000024481 | 1.000004399 | 1.000059891 | HBM_VALIDATED | none recorded / none recorded / none recorded |
| 5 | tma | 8 | 64 | 1.000031372 | 1.000012706 | 1.000049859 | HBM_VALIDATED | none recorded / none recorded / none recorded |

`none recorded` in the column above is an observed result, not missing data: for 6 of the 6 profiled cases the upstream `diagnostic_flags` field was **present and its flag set was empty** in every campaign, so the profiler recorded no diagnostic. diagnostic_flags is a preserved upstream P1.4 field, not a P4.3 computation. For a profiled case the field is always present: 'present_and_empty' means it was present and its flag set was empty -- the profiler raised no diagnostic, rendered 'none recorded' -- and 'present_and_non_empty' means a flag such as READ_AMPLIFICATION was recorded and is surfaced verbatim. An empty flag set is a clean result and is never converted into a warning. 'unavailable_not_profiled' is the distinct state of the twelve configurations that were never profiled, whose actual HBM/DRAM traffic is unavailable from the collected evidence. The canonical 'not_applicable' statistic cells of an empty flag set therefore record an observed absence of diagnostics, never an unavailable quantity.
...
**How to read the UMMA cross-campaign spread. The primary metric is derived from integer-cycle-quantized %clock64 measurements whose within-campaign variation is approximately zero, so the campaign-level medians frequently land on identical values and the cross-campaign sample standard deviation and coefficient of variation are frequently exactly zero. Those statistics are mathematically valid and are preserved unchanged, and no cross-campaign coefficient of variation exceeded the frozen review threshold. A zero coefficient of variation here is nevertheless a property of a cycle-quantized instrument and must not be read as proof of extraordinary independent reproducibility: it reports that no review threshold was exceeded, not that reproducibility was demonstrated under strain. Where the min-max whisker spans zero, or spans less than the plotted marker, the whisker is still drawn at its true length and is hidden by the marker.**
...
**How to read these candidates.** The tested grid is exactly 4, 16, 64, 256 tested depths, so `256` is the **upper boundary of the evaluated grid**, not a measured architectural limit: a candidate saturation point is an upstream selection made strictly within the tested grid, never a measured architectural limit and never a universal saturation threshold. Here every group's candidate equals 256, the largest tested depth, and the operation-and-cycle-derived FLOP/cycle/SM is still rising across the final tested step in every group, so no plateau was observed within the tested range. When the selected candidate equals the largest tested grid point, the selection has reached the boundary of the evaluated range: the tested grid cannot distinguish a genuine saturation point there from one that lies beyond the largest value tested, and every campaign evaluates the same frozen grid, so agreement between the three campaigns on a candidate that sits at that grid's upper boundary is not by itself evidence that a saturation point was independently located.
...
analysis_code_commit = 2ef1ac52907c407dd43c41661382fc8d5673cce4
```
